...
#define SERVO_PIN 33 // Chân cho servo
...
bool doorIsOpen = false; // Trạng thái cửa
...
// Hàm mở cửa
void openDoor() {
  if (!doorIsOpen) { // Chỉ mở nếu cửa chưa mở
    doorServo.attach(SERVO_PIN); // Gắn lại servo
    doorServo.write(180); // Quay servo 90 độ (theo yêu cầu)
    doorIsOpen = true;
    Serial.println("Door opened to 90 degrees");
  }
}
...
void drawAuthScreen() {
  ...
  tft.fillRect(RFID_BUTTON_X, RFID_BUTTON_Y, PASS_BUTTON_W, PASS_BUTTON_H, TFT_NAVY);
  tft.drawRect(RFID_BUTTON_X, RFID_BUTTON_Y, PASS_BUTTON_W, PASS_BUTTON_H, TFT_WHITE);
  tft.setCursor(RFID_BUTTON_X + 10, RFID_BUTTON_Y + 15);
  tft.println("Quet the RFID");
  tft.fillRect(FINGER_BUTTON_X, FINGER_BUTTON_Y, PASS_BUTTON_W, PASS_BUTTON_H, TFT_NAVY);
  tft.drawRect(FINGER_BUTTON_X, FINGER_BUTTON_Y, PASS_BUTTON_W, PASS_BUTTON_H, TFT_WHITE);
...
void addFingerprint() {
  if (fingerprintCount >= MAX_FINGERPRINTS) {
    ...
    Serial.println("Fingerprint list full");
    return;
  }
...
void deleteRFID(int id) {
  if (id < 0 || id >= rfidCount) {
    showMessage("ID khong hop le");
    Serial.println("Invalid RFID ID: " + String(id));
    return;
  }
  int index = -1;
  for (int i = 0; i < rfidCount; i++) {
  ...
  if (index == -1) {
    ...
    Serial.println("RFID ID " + String(id) + " not found");
    return;
  }
...
void setup() {
  Serial.begin(115200);
  delay(1000);
  Serial.println("XPT2046 Touchscreen, AS608 Fingerprint, MFRC522 RFID, and Servo Interface");
...
void loop() {
  if (currentMode == TOUCH_MODE) {
    if (ts.touched()) {
      ...
      if (p.z >= 400 && p.z <= 3000) {
        ...
        int16_t x = map(p.x, 200, 4000, 0, 320);
        int16_t y = map(p.y, 200, 4000, 0, 240);
        x = constrain(x, 0, 319);
        y = constrain(y, 0, 239);
          ...
          if (y >= KEY_Y1 && y <= KEY_Y1 + KEY_H) {
            ...
            if (x >= KEY_X1 && x <= KEY_X1 + KEY_W) password = "";
              ...
              if (password == storedPassword) {
                ...
                Serial.println("Incorrect old password");
              }
              return;
            }
          }
          ...
          if (y >= KEY_Y1 && y <= KEY_Y1 + KEY_H) {
            ...
            if (x >= KEY_X1 && x <= KEY_X1 + KEY_W) password = "";
              ...
              if (password == storedPassword) {
                ...
                showMessage("Mat khau sai");
                Serial.println("Incorrect password for add fingerprint");
              }
              return;
            }
          }
          ...
          if (y >= KEY_Y1 && y <= KEY_Y1 + KEY_H) {
            ...
            if (x >= KEY_X1 && x <= KEY_X1 + KEY_W) password = "";
              ...
              if (password == storedPassword) {
                ...
                showMessage("Mat khau sai");
                Serial.println("Incorrect password for add RFID");
              }
              return;
            }
          }
        ...
        // Xử lý xóa RFID
          ...
          for (int i = 0; i < rfidCount && i < 12; i++) {
            ...
            if (x >= x_pos && x <= x_pos + ID_W &&
              ...
              Serial.println("Selected RFID ID: " + String(selectedRFIDID) + " at index: " + String(i));
              currentScreen = CONFIRM_DELETE_RFID;
              drawConfirmDeleteScreen("ID " + String(selectedRFIDID), false);
              return;
            }
          }
          Serial.println("No RFID ID selected at X: " + String(x) + ", Y: " + String(y));
        }
        // Xử lý xác nhận xóa vân tay
          ...
          if (x >= CONFIRM_OK_X && x <= CONFIRM_OK_X + CONFIRM_OK_W &&
            ...
            Serial.println("Confirm delete fingerprint ID: " + String(selectedFingerprintID));
            ...
            Serial.println("Cancel delete fingerprint");
            currentScreen = DELETE_FINGERPRINT;
            drawDeleteListScreen("Danh sach van tay:", true);
            return;
          }
        ...
        // Xử lý xác nhận xóa RFID
          ...
          if (x >= CONFIRM_OK_X && x <= CONFIRM_OK_X + CONFIRM_OK_W &&
            ...
            return;
                     ...
                     y >= CONFIRM_CANCEL_Y && y <= CONFIRM_CANCEL_Y + CONFIRM_OK_H) {
            Serial.println("Cancel delete RFID");
            currentScreen = DELETE_RFID;
            drawDeleteListScreen("Danh sach RFID:", false);
            return;
          }
      ...
      if (checkRFID(uid)) {
        if (currentScreen == SCAN_RFID) {
          ...
          if (checkMasterRFID(uid)) {
            ...
            Serial.println("Incorrect Master RFID");
          }
        }
      }
  ...
  if (currentScreen == SCAN_FINGERPRINT || currentScreen == ADD_FINGERPRINT) {
    if (currentScreen == ADD_FINGERPRINT) {
      addFingerprint();
    } else if (currentScreen == SCAN_FINGERPRINT) {
      int fingerID = 0;
      unsigned long startTime = millis();
      bool fingerDetected = false;
      drawScanScreen("Quet van tay..."); // Hiển thị giao diện quét
      while (millis() - startTime < 10000) { // Chờ 10 giây
        if (checkFingerprint(fingerID)) {
          fingerDetected = true;
          if (checkStoredFingerprint(fingerID)) {
            ...
            Serial.println("Incorrect fingerprint");
          }
          break;
        }
        delay(50); // Tránh CPU chạy quá tải
      }
      if (!fingerDetected) {
        showMessage("Khong phat hien van tay");
        Serial.println("No fingerprint detected");
